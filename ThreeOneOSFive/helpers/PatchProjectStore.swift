...
private func enforceMinimumInjectionDuration(startedAt: UInt64) async {
    let minimumDuration: UInt64 = 2_000_000_000
    let elapsed = DispatchTime.now().uptimeNanoseconds - startedAt
    guard elapsed < minimumDuration else { return }
    try? await Task.sleep(nanoseconds: minimumDuration - elapsed)
}

private enum BuiltInResourceLocator {
    static func url(for resourceName: String) -> URL? {
        let directories: [String]
        if resourceName.hasPrefix("FF2022_") {
            directories = ["FF2022", "Resources/FF2022"]
        } else {
            directories = ["FF", "Resources/FF"]
        }
        ...
        return Bundle.main.url(forResource: resourceName, withExtension: "3105")
    }
}
...
final class PatchProjectStore: ObservableObject {
    ...
    func applyBundledPackages(
        named resourceNames: [String],
        completion: @escaping (Result<Void, Error>) -> Void
    ) {
        guard !isBusy, !resourceNames.isEmpty else { return }

        let packageURLs = resourceNames.map { BuiltInResourceLocator.url(for: $0) }
        guard let missingIndex = packageURLs.firstIndex(where: { $0 == nil }) else {
            isBusy = true
            let startedAt = DispatchTime.now().uptimeNanoseconds
            Task.detached(priority: .userInitiated) { [weak self] in
                do {
                    for packageURL in packageURLs.compactMap({ $0 }) {
                    ...
                    await enforceMinimumInjectionDuration(startedAt: startedAt)
                    await MainActor.run {
                        self?.isBusy = false
                        completion(.success(()))
                    }
        ...
        _ = missingIndex
        completion(.failure(PatchPackageError.invalidProject))
    }

    func applyBundledPackage(named resourceName: String) {
        guard !isBusy else { return }
        applyBundledPackages(named: [resourceName]) { [weak self] result in
            switch result {
            case .success:
    ...
    private nonisolated static func persistImportedPackage(
        ...
        if summary.isPasswordProtected {
            /*
             A stale/unavailable Keychain item must not turn an import into a
             generic corruption error. Fall back to the normal unlock flow so
             the package password can be tried again.
             */
            do {
                if let key = try PatchKeyStore.load(for: summary) {
                    do {
                        ...
                        guard case .invalidPasswordOrCorruptedPackage = error else {
                            throw error
                        }
                    }
                }
            } catch let error as PatchPackageError {
                guard case .keychainFailed = error else {
                    throw error
                }
            }
