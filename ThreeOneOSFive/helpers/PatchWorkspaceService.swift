...
enum PatchWorkspaceService {
    ...
    private static func workspaceTarget(
        ...
        let canonicalWorkspaceRoot = PatchPathValidator.canonicalFileURL(workspaceRoot)
        let bundleRoot = PatchPathValidator.canonicalFileURL(
            canonicalWorkspaceRoot.appendingPathComponent(bundle, isDirectory: true)
        )
        let components = path.split(separator: "/").map(String.init)
        var target = bundleRoot
        for (index, component) in components.enumerated() {
            target.appendPathComponent(
                component,
                isDirectory: isDirectory || index < components.count - 1
            )
        }
        /*
         The workspace root and bundle directory are created by this method.
         The path has already been canonicalized and is appended one component
         at a time, so a valid path cannot escape this freshly-created bundle.
         Avoid comparing textual URL paths here: iOS may expose the same
         sandbox through both /var and /private/var.
         */
        return target
    }

    private static func relativePath(for item: URL, bundleRoot: URL) throws -> String {
        let rootPath = PatchPathValidator.canonicalFileURL(bundleRoot).path
        let itemPath = PatchPathValidator.canonicalFileURL(item).path
