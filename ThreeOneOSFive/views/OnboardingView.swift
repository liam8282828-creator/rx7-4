            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    ForEach(ExploitSupportPolicy.verifiedIOS27Builds, id: \.build) { v in
                        HStack {
                            Text(v.publicBeta == nil
                                 ? language.text("onboarding.developer_beta", Int64(v.beta))
                                 : language.text(
                                     "onboarding.developer_public_beta",
                                     Int64(v.beta),
                                     Int64(v.publicBeta ?? 0)
                                 ))
    ...
    private var installPage: some View {
        VStack(spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 10) {
                 ...
                 installBullet(icon: "exclamationmark.triangle.fill", text: language.text("onboarding.install_jailbreak"), color: AppTheme.accent)
            }
            .padding(14)
             .background(Color(uiColor: .secondarySystemBackground), in: RoundedRectangle(cornerRadius: 10, style: .continuous))
            .padding(.horizontal, 20)
