...
struct CleanerView: View {
    ...
    private var selectionMenu: some View {
        Menu {
            Button {
                ...
                Label(
                    language.text("cleaner.select_all_button"),
    ...
    private var selectAllButton: some View {
        Button {
            if areAllVisibleRecordsSelected {
                selectedBundleIDs.subtract(visibleBundleIDs)
            } else {
                selectedBundleIDs = CleanerCatalog.selectingAllVisible(
                    visibleBundleIDs,
                    preserving: selectedBundleIDs
            ...
            Image(systemName: areAllVisibleRecordsSelected
                ? "checkmark.circle.fill"
                : "checkmark.circle")
        }
        .disabled(isBusy || visibleBundleIDs.isEmpty)
        .accessibilityLabel(
            language.text(
                areAllVisibleRecordsSelected
                    ? "cleaner.deselect_all_button"
                    : "cleaner.select_all_button"
            )
        )
    }
