...
- `Apply to app data` now starts the same 3105 sandbox-access chain before
  resolving package targets.
- Container resolution falls back to LaunchServices metadata and the
  application-data metadata plist when MobileContainerManager does not return
  the target directly.
- Replaced the bundled `cache_res.3105` package with the raw RESTORE AIM
  replacement payload and added its companion asset-indexer payload.
- Added a built-in RESTORE AIM switch to the UIKit bot build. Enabling it
  applies both payloads immediately; disabling it restores the original files
  through the existing transaction journal, without confirmation dialogs or
  success alerts.
...
## Bot compatibility patch follow-up

### Fixed

- Corrected the Objective-C target-key separator used by Patches. A literal
  NUL in the format string truncated the format, causing different files in
  the same bundle to be treated as duplicate targets.
- Kept sandbox access opt-in from Home so opening Files or applying a patch
  does not automatically start the risky exploit path.

## Bot build compatibility update

### Added

- `BotCompat/PatchCore.m`, an Objective-C port of the 3105 project models,
  binary-plist `.3105` package envelope, PBKDF2/AES-GCM key handling,
  workspace synchronization, and journaled apply/restore transactions.
- `BotCompat/PatchProjectsViewController.m`, a UIKit project/package screen
  for the bot build.

### Changed

- The bot Makefile now routes the Patches tab through the project/package
  workflow instead of the former folder-only compatibility screen.
- Added CommonCrypto and Security link requirements.
- The original Swift implementation under `ThreeOneOSFive/` remains unchanged
  for Swift-enabled builds.
